function [graph, dnc, cols, unfaithful, triangles,pvalue2] = R0_conservative(graph, sepSet, test, dataset, alpha, maxK, screen)
...
for iTriple =  1:numTotalUnshieldedTriples
    ...
    elseif  sepSet(curTriple(1), curTriple(3), curTriple(2)) == 0
        [is_collider, p]=isConservativeCollider(curTriple, graph,test, dataset, alpha, maxK);
        if is_collider
            if p<pvalue2(curTriple(1),curTriple(3)) && p< pvalue2(curTriple(2),curTriple(3)) 
                pvalue2(curTriple(1),curTriple(2))=p;
                pvalue2(curTriple(2),curTriple(1))=p;
                pvalue2(curTriple(2),curTriple(3))=p;
                pvalue2(curTriple(3),curTriple(2))=p;
            end
            ...
            if screen
             ...
             %fprintf ('collider %d->%d<-%d; p-value =%d \n', curTriple(1), curTriple(2), curTriple(3), p); 
        else
            numUnfaithful =  numUnfaithful+1;
            unfaithful(numUnfaithful, :) =  curTriple;
            if screen
...
function [isCollider, initial_p] =  isConservativeCollider(triple, graph,test, dataset, alpha, maxK)
...
%                end   
       ...
       %   initial_p=initial_p+p;
           num_p=num_p+1;

        
        %fprintf ('triple %d-%d-%d ; p-value =%d \n' ,a,b,c,p);
        
    end% end for iCondSet
end % end while
end % end for iEndpoint 
mean_p=initial_p/num_p;
