...
% starting values
omega =diag(diag(covMat),0); % retuen diagonal of cov ii
beta = eye(nVars);
...
while true
    ...
    ricf(iter).beta = beta;
    %fprintf('iteration %d, omegava= %d beta %d------------\n',iter, omega, beta);
% for each variable
    for iVar =1:nVars
       ...
       if isempty(iSp)
           if ~isempty(iPar)
               if iter==1;
                   beta(iVar, iPar) = -covMat(iVar, iPar)*inv(covMat(iPar, iPar)); %Bij=-Covijx(Covjj)-1
                   omega(iVar, iVar) = covMat(iVar, iVar) + beta(iVar, iPar)*covMat(iPar, iVar); %Oii=Covii+BijCovji
               end
           end
