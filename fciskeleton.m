function [pag, sepSet,pvalues, maxSepSet, nTests] = fciskeleton(dataset, heuristic, alpha, maxK,  pdSep, verbose)
...
for X  =  1:nVars;
    ...
    for Y =  X+1:nVars;
        ...
        [p, s, exitflag] = fisher(X, Y, [], dataset); 
        if p>alpha
            pag(X, Y) = 0;
            pag(Y, X) = 0;
            flagPag(X, Y) = 1;
    ...
    for iEdge =1:length(ord);
       ...
       [condsets, uniqueOrd]= unique(condsets, 'rows', 'stable');
        ...
        for iCS = 1:size(condsets, 1)
            ...
            [p, ~, exitflag] = fisher(X, Y, condset, dataset);
            nTests =nTests+1;
            if p>alpha              
                pag(X, Y) = 0; pag(Y, X) = 0;
